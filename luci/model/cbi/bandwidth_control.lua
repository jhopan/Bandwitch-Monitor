...
local used=dev:option(DummyValue,"used",translate("Usage / quota")); function used.cfgvalue(self,s) local mac=self.map.uci:get("bandwidth-control",s,"mac") or ""; local _,_,n=usage(mac); local q=quota_value(self.map.uci:get("bandwidth-control",s,"quota_gb")); local limit=q*1073741824; local pct=q>0 and math.floor(n/limit*100) or 0; return string.format("%s / %s GB (%d%%), left %s",fmt(n),q,pct,fmt(math.max(0,limit-n))) end
